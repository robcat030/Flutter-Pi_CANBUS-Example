...
class Ecu:
	def __init__(self, can_interface="can0", ecuid=0, bs=0, stmin=0xff, timeout=5):
	...
	def UDSRequestDownload(self, addr, size, fmt=0x00, aslen=0x44): #fmt is encryption/compression, aslen is address size and length
		slen = aslen >> 4 #byte shift aslen to the right by 4 places, which is 4
		alen = aslen & 0x0f

		self.send(bytearray([0x34, fmt, aslen]) + addr.to_bytes(alen, 'big') + size.to_bytes(slen, 'big')) # send 34 00 44 + address + size
		msg = self.recv() # receive message

		if msg and msg[0] == 0x74: #if 74, send
			return msg[2:] # [2:] means everything but the last two bytes 04 74 20 08
		return False
